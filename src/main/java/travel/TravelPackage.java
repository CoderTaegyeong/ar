...
public class TravelPackage extends AppView {
	...
	public void showPackageList() {
		...
		int rows = Math.max(2, travel.getPackageList().size()/2);
		int cols = 2;
		center.removeAll();
		...
		for(int i=0; i < rows * cols; i++) {
			if(i >= packageList.size()) return;
				...
				final int a = pack.getId();
				Gui.addBorderOnEnterMouse(imagePanel.getPanel(), b->travel.openDetail(a));
