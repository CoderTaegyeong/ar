...
import entity.TicketDTO;
	...
	public class SelectSeat extends AppView implements ActionListener, ItemListener {
		...
		private TicketDAO ticketDao; // 결제하면 ticketdb에 등록
		...
		// 이전 창에서 선택한 사람 수 만큼의 좌석 Label이 만들어짐.
		...
		public void buildGUI() {
			...
			//ROW와 COL을 정해주기 전 DB에 ROW와 COL의 MAX값을 가져올 수 있도록 INSERT
			
			//좌석 만들기 전 FIELD의 ROW와 COL을 정해준다
			getRowAndCol();
			// 행	
			for (int i = 0; i < row; i++) {
